...
class SonnenBatteryAPI{
	...
	async setChargingPower(watts){
		try {
            const response = await this.instance.post(`/setpoint/charge/${watts}`);
            return response.data;
        } catch (error) {
            console.error('Error setting charge point:', error);
            throw error;
        }
	}
	async setDischargingPower(watts){
		try {
			const response = await this.instance.post(`/setpoint/charge/${watts}`);
			return response.data;
		} catch (error) {
